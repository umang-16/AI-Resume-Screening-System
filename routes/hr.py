...
def download_analysis(app_id):
    ...
    report_lines = [
        ...
        f"==================================================",
        f"Candidate:  {application.student.name}",
        f"Job Role:   {application.job.title}",
        f"Match:      {application.match_score}%",
        f"Date:       {application.applied_at.strftime('%Y-%m-%d %H:%M:%S')}",
        f"--------------------------------------------------",
        f"\n[ VERIFIED CORE COMPETENCIES ]",
        *([f" - {skill.title()}" for skill in matched_skills] if matched_skills else [" - None detected."]),
        f"\n[ MISSING PREREQUISITES ]",
        *([f" - {skill.title()}" for skill in missing_skills] if missing_skills else [" - None. Perfect Match!"]),
        f"\n[ SURROUNDING TECH STACK / EXTRA SKILLS ]",
        *([f" - {skill.title()}" for skill in extra_skills] if extra_skills else [" - None detected."]),
        f"\n==================================================",
        f"Generated by AIPlacement System"
    ]
    ...
    return Response(
        report_text,
        mimetype="text/plain",
        headers={"Content-disposition": f"attachment; filename=Analysis_Report_{application.student.name.replace(' ', '_')}.txt"}
    )
